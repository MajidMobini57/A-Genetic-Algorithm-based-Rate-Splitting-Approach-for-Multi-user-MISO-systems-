...
hold on
s=[0:2:40]
c=[ 2.5223 3.2694 3.8722 4.9104 5.7615 6.57335 ...
    7.4323 8.2186 8.4491 8.7264 8.9911 ...
    9.3047 9.7985 10.38 10.87 11.46 12.1371 ...
    12.76 13.64 13.9870 14.7266]
    
d=[ 2.56 3.3 3.96 4.96 5.86 6.66 7.64 ...
    8.18 8.73 9.33 9.55 9.99 10.4 10.68 ...
    11.15 12.15 12.74 13.2 14.11 14.5 15.41];
figure; hold on;
...
legend('Perfect-ZFBF','RVQ-ZFBF','Approximated RSS','GA-based RSS','GA-based2 RSS');


 sum_rate_loss_rss_ga=sum_rate_Pzf-c';
 sum_rate_loss_rss_ga2=sum_rate_Pzf-d';

figure; hold on;
plot(SNR_values, sum_rate_loss_zf, 'r');
plot(SNR_values, sum_rate_loss_rss, '^-y');
 plot(SNR_values, sum_rate_loss_rss_ga, '^-k');
 plot(SNR_values, sum_rate_loss_rss_ga2, '^-b');

% plot(SNR_values, sum_rate_loss_theor_zf, 'b');
xlabel('SNR'); ylabel('Sum Rate Loss(bps/hz)');
title('Sum rate loss, M= 4, B= 10');
legend('ZFBF RVQ MC','\Delta(R_s)( t_e_q^s)','GA-based','GA-based2');
