...
QuantizType=1;%Random vector quantization
    ...
    for its = 1:MaxIter
        H = 1/sqrt(2) * (randn(K, M) + 1i * randn(K, M)); %channel
            QuantH = quantiz_channels(H, regions, snr, QuantizType, CQIType);
%                          rate_temp2 = compute_ZF_rate(QuantH,H, P);
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% start zero forcing     
num_users = size(H, 1);
%  if B<= (M-1)*log2(snr)-(M-1)*log2(2*(M-1)/M)-(M-1)*log2(exp(1)-1)
% 
%  r=1/(snr*M/(2*(M-1))*2^(-B/(M-1))+2-exp(1));
% % r=2*(M-1)/(P*M)*2^(B/(M-1));
% else
% r=1;
% end
...
Wc(:,1)= sqrt(p(1)) * Wc(:,1)/norm(Wc(:,1)); 


    zf_F(:,1) =  (zf_F(:,1)/norm(zf_F(:,1)))*sqrt(Pu1) ; 
    zf_F(:,2) =  (zf_F(:,2)/norm(zf_F(:,2)))*sqrt(Pu2) ; 
